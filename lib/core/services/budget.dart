...
class BudgetService {
  ...
  Future<void> setBudgetForCategory({
    ...
    if (amount < 0) return;
    final existing =
        await (_db.select(_db.budgetsTb)..where(
              (t) =>
                  t.year.equals(year) &
                  t.month.equals(month) &
                  t.transactionCategoryId.equals(categoryId),
  ...
  Future<void> setMonthlyBudget({
    ...
    final existing =
        await (_db.select(_db.monthlyBudgetsTb)
              ..where((t) => t.year.equals(year) & t.month.equals(month)))
            .getSingleOrNull();
  ...
  Future<int> getTotalSpentForMonth({
    ...
    return await _transactionService.calculateTotalAmountInMonth(
      year: year,
      month: month,
  ...
  getCategoryBreakdown({
    ...
    final transactions = await _transactionService.getTransactionsFiltered(
      year: year,
      month: month,
      transactionType: TransactionType.expense.typeIndex,
      categoryId: categoryId,
