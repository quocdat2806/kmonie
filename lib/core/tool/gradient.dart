...
class GradientHelper {
  ...
  static List<String> generateSmartGradientColors() {
    ...
    final baseColor = HSLColor.fromAHSL(1, baseHue, baseSaturation, baseLightness).toColor();

    final hueOffset = (random.nextBool() ? 1 : -1) * (15 + random.nextDouble() * 15);
    final secondHue = (baseHue + hueOffset) % 360;
    final secondLightness = (baseLightness + (random.nextDouble() * 0.1 - 0.05)).clamp(0.3, 0.8);
    final secondSaturation = (baseSaturation + (random.nextDouble() * 0.1 - 0.05)).clamp(0.5, 1.0);
    final secondColor = HSLColor.fromAHSL(1, secondHue, secondSaturation, secondLightness).toColor();
