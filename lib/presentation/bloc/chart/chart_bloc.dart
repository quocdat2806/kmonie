...
class ChartBloc extends Bloc<ChartEvent, ChartState> {
  ...
  ChartBloc(this.transactionService, this.categoryService)
      ...
      switch (data.event) {
        ...
        case AppEvent.deleteTransaction:
          if (data.payload is int) {
            ...
            final hasTransaction = state.localTransactions.any(
  ...
  Future<void> _onRefreshChart(
    ...
    try {
      ...
      if (state.selectedPeriodType == IncomeType.month &&
        ...
        transactions = await transactionService.getTransactionsInYear(
          state.selectedYear!,
        );
      }
      ...
      final indices = List<int>.generate(chartData.length, (i) => i)
        ..sort((a, b) => chartData[b].value.compareTo(chartData[a].value));
      ...
      emit(
        state.copyWith(
          ...
          chartCategoryIds: sortedCatIds,
          categoryGradients: categoryGradients,
          localTransactions: transactions,
          localCategoryTotals: categoryTotals,
          localCategoryGradients: categoryGradients,
          isLoading: false,
        ),
      );
    } catch (e) {
  ...
  void _onAddTransactionLocal(
    AddTransactionLocal event,
    Emitter<ChartState> emit,
  ) {
  ...
  void _onUpdateTransactionLocal(
    UpdateTransactionLocal event,
    Emitter<ChartState> emit,
  ) {
    ...
    final transactionIndex = state.localTransactions.indexWhere(
  ...
  void _onDeleteTransactionLocal(
    DeleteTransactionLocal event,
    Emitter<ChartState> emit,
  ) {
