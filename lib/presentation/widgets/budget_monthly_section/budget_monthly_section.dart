...
class BudgetMonthlySection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double progress = moneyBudget > 0 ? (totalSpent / moneyBudget).clamp(0.0, 1.0) : 0.0;
    final bool isOverBudget = remaining < 0;
    final backgroundColor = isOverBudget ? AppColorConstants.red : AppColorConstants.primary;
    final valueColor = isOverBudget ? AppColorConstants.red.withValues(alpha: 0.7) : AppColorConstants.grey;
      ...
      children: [
            ...
            children: [
              Transform.rotate(
                ...
                child: SizedBox(
                  width: AppUIConstants.extraLargeContainerSize,
                  height: AppUIConstants.extraLargeContainerSize,
                  child: CircularProgressIndicator(value: progress, strokeWidth: AppUIConstants.strokeWidthSmall, backgroundColor: backgroundColor, valueColor: AlwaysStoppedAnimation<Color>(valueColor), strokeCap: StrokeCap.butt),
                ),
              ),
              Center(
                child: Column(
        ...
        Expanded(
          child: Column(
            ...
            children: [
              _buildBudgetInfoItem('${AppTextConstants.remaining} :', FormatUtils.formatCurrency(remaining.abs()), isOverBudget: isOverBudget, isNegative: isOverBudget),
              _buildBudgetInfoItem('${AppTextConstants.budget} :', FormatUtils.formatCurrency(moneyBudget)),
              _buildBudgetInfoItem('${AppTextConstants.spent} :', FormatUtils.formatCurrency(totalSpent), isOverBudget: isOverBudget),
            ],
          ),
        ),
      ],
